 ...
 */
export const createChartConfig = (type: string, title: string, realData: any[] | null = null) => {
  ...
  switch (type) {
        ...
        series: [
          {
            ...
            tooltip: { renderer: ({ datum, xKey, yKey }: any) => ({ content: `${datum[yKey]}` }) }
          }
        ],
        axes: [
          {
    ...
    case 'Gauge':
      ...
      if (isUsingRealData && chartData && chartData.length > 0) {
        ...
        // Dummy data fallback
        currentValue = Number(chartData[chartData.length - 1].value);
      }
