<template>
      ...
      <v-main>
        <div class="p-8">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            >
              <h3 :class="['text-lg font-semibold', isDark ? 'text-white' : '']">{{ project.name }}</h3>
              <p :class="['text-sm mt-8', isDark ? 'text-gray-400' : 'text-gray-400']">{{ $t('dashboard.owner') }}: {{ project.owner.username }}</p>
            </div>
            ...
            >
              ...
              <p :class="['mt-2', isDark ? 'text-gray-300' : 'text-gray-600']">{{ $t('dashboard.newProject') }}</p>
            </div>
          </div>
        </div>
      </v-main>
  ...
  <div v-if="showCreate" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div :class="['p-6 w-full max-w-md rounded-xl shadow-xl', isDark ? 'bg-gray-800' : 'bg-white']">
      <h2 :class="['text-xl font-semibold mb-4', isDark ? 'text-white' : '']">{{ $t('dashboard.createProject') }}</h2>
